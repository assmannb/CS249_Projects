...
public class ContainerBoxClass
{
    ...
    /**
     * Uses -1 as constant for reporting no boxes returned from
     * findNexUnusedBoxIndex
     */
    ...
    /**
     * Boolean display flag to indicate if recursive process should be displayed
     */
    ...
    public void displayField()
    {
        ...
        if( displayFlag )
        {
            ...
            for( rowIndex = 0; rowIndex < containerBoxHeight; rowIndex++ )
            {
                ...
                {
                    System.out
                            .print( containerBoxField[ rowIndex ][ colIndex ] );
                }
    ...
    /**
     * Searches for unused box starting at the given index of the list of boxes
     * to be packed
     * 
            ...
            BoxClass testBox )
    {
        ...
        for( rowIndex = testLocation
                .getYPos(); rowIndex > heightLimit; rowIndex-- )
        {
            for( colIndex = testLocation
                    .getXPos(); colIndex < widthLimit; colIndex++ )
            {
                if( containerBoxField[ rowIndex ][ colIndex ] != DEFAULT_FIELD_CHAR )
                {
                    return false;
            ...
            int clearFlag )
    {
        ...
        for( rowIndex = boxLocation
                .getYPos(); rowIndex > heightLimit; rowIndex-- )
        {
            for( colIndex = boxLocation
                    .getXPos(); colIndex < widthLimit; colIndex++ )
            {
 ...
 */
/*
 * private static void waitForEnter( String message, Object...args ) { Scanner
